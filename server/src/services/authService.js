...
const sqlUser = `select u.id, u.email, u.senha, u.status from usuarios u where u.email = $1`
...
export async function AuthLogin({ password, email }) {
  try {
    ...
    if (user.rowCount === 0) return returnCode(true, 401, 'Credenciais inválidas')
    ...
    if (!compare) return returnCode(true, 401, 'Senha inválida')
